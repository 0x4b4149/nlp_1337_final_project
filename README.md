...
4. **產出結果**：
   ...
   * **模擬網頁**：讀取資料庫中的詐騙案例文案進行分析與生成後，自動注入美化的現代 CSS 樣式，並為每筆資料分別輸出獨立檔案（例如 **`output_ad_<廣告id>.html`**）。

> [!IMPORTANT]
> 這裡產出的 `output_ad_<廣告id>.html` 相當於第三部分網頁伺服器實際運作時所使用的 `mock_scam.html` 樣式範本。
