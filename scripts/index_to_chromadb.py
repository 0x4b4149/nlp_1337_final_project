...
METADATA_FILE = os.path.join(PROJ_ROOT, "ad_labels_metadata.json")
...
def prepare_documents(metadata_map, existing_ids, proj_root):
    ...
    for ad_id, info in metadata_map.items():
        ...
        json_path = os.path.join(proj_root, info["json_path"])
...
def main():
    ...
    # 5. 準備需要寫入的數據
    documents, metadatas, ids = prepare_documents(metadata_map, existing_ids, PROJ_ROOT)
